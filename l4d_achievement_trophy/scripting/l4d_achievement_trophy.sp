...
2.5 (28-Sep-2019)
	- fixed tank dead in l4d1 by Harry
...
void CreateHat(int client)
{
	...
	if(g_bLeft4Dead2)
	{
		if (!IsModelPrecached(MODEL_mustachio))
			PrecacheModel(MODEL_mustachio, true);
		SetEntityModel(entity, MODEL_mustachio);
	}
	else
	{
		if (!IsModelPrecached(MODEL_CONE))
...
void CreatePropGlow(int client, int entity)
{
	if(g_bLeft4Dead2)
	{
		...
		SetEntProp(g_iBoomerHatGlow[client], Prop_Send, "m_glowColorOverride", GetColor(255,255,255));
